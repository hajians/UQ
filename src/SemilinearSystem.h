  ...
  SemilinearSystem(double D_SpeedOfSound, double D_TerminalTime,
	...
	private:
		...
		*	on the actual evolution of Q have to be stored, the values of this array are overwritten in every time step.	*/
		...
		///	Values of P in the right ghost cells for all time steps.
		...
		///	Integral averages of Lambda in all of the cells including the ghost cells on the boundaries.
		/**	Pointer to Double Array that stores the integral averages of Lambda in every interior cell and the linear extrapolation depending on the two left- and right-most
		*	interior values on the ghost cells. The elements on this array are re-defined  in every .Run call of the object since the runs only difer in the choice of Lambda.	*/
		double 	* DA_P_Lambda_AV;				//	Pointer on Lambda-Averages on every cell of the discretization
		///	Coefficients of the Lambda expansion defining the Lambda in a certain .Run call.
		/**	Pointer to Double Array that stores the expectational value of Lambda on the first component and the coefficients of the Lambda-expansion on the following components.	*/
